...
export default function ReconciliationPage() {
  ...
  const [dropRateOpen, setDropRateOpen] = useState(false);
    ...
    <div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <header className="mb-6 flex items-end justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              CallRail × Spark contact reconciliation
            </p>
          </div>

          <div className="flex items-center gap-2">
            {data && (
              <span className="text-xs text-gray-500 dark:text-gray-400 hidden sm:inline mr-2">
            ...
            {isAdmin && (
              <div className="relative" ref={reportsRef}>
                <button
                  ...
                  className="flex items-center gap-2 px-3.5 py-2 rounded-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 text-sm font-medium text-gray-700 dark:text-gray-200 shadow-sm transition-all"
                ...
                {reportsOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-lg z-50 py-1">
            ...
            <button
              onClick={() => fetchData()}
              disabled={loading}
              className="flex items-center gap-2 px-3.5 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white text-sm font-medium shadow-sm transition-all disabled:opacity-50"
            >
        ...
        <div className="flex flex-wrap items-center gap-3 mb-8">
          <div className="flex items-center bg-white dark:bg-gray-800 rounded-xl p-1 ring-1 ring-gray-200 dark:ring-gray-700">
            {DATE_PRESETS.map(p => (
              <button
                key={p.label}
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
            ...
            <input
              ...
              className="bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-gray-400 dark:focus:ring-gray-600"
            />
            <span>&ndash;</span>
            <input
              type="date"
            ...
            {activePreset === 'custom' && (
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-sm font-medium hover:bg-gray-700 dark:hover:bg-gray-200 transition-colors"
              >
                Go
              </button>
            )}
        ...
        {loading && !data && (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            <div className="relative">
              <div className="w-16 h-16 rounded-full border-2 border-gray-200 dark:border-gray-800 border-t-gray-900 dark:border-t-white animate-spin" />
              <Zap className="w-6 h-6 text-gray-900 dark:text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
            </div>
            <div className="text-center">
              <p className="text-lg font-medium text-gray-900 dark:text-gray-100">Cross-referencing systems</p>
        ...
        {data && (
          <>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl ring-1 ring-gray-200 dark:ring-gray-700 mb-8 overflow-hidden">
              <button
                type="button"
                onClick={() => setDropRateOpen(o => !o)}
                aria-expanded={dropRateOpen}
                aria-controls="drop-rate-panel"
                className="w-full flex items-center justify-between gap-4 px-6 py-4 text-left hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors cursor-pointer"
              >
                <div className="flex items-center gap-3">
                  <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Drop Rate by Source</h2>
                  {data.sourceStats.length > 0 && (
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                      {data.sourceStats.length} source{data.sourceStats.length === 1 ? '' : 's'}
                    </span>
                  )}
                </div>
                <ChevronDown className={`w-4 h-4 text-gray-500 dark:text-gray-400 transition-transform ${dropRateOpen ? 'rotate-180' : ''}`} />
              </button>
              {dropRateOpen && (
                <div id="drop-rate-panel" className="px-6 pb-6 pt-2 space-y-3 border-t border-gray-200 dark:border-gray-700">
                  {data.sourceStats.map(s => (
                    <div key={s.source} className="flex items-center gap-4">
                      <div className="w-40 flex items-center gap-2 flex-shrink-0">
                        ...
                        <span className="text-sm font-medium truncate text-gray-700 dark:text-gray-200">{s.source}</span>
                      </div>
                      <div className="flex-1 flex items-center gap-2">
                        ...
                        <div className="w-24 flex items-center gap-1.5 flex-shrink-0">
                          <span className="text-xs text-emerald-600 dark:text-emerald-400 font-mono">{s.inSpark}</span>
                          <span className="text-xs text-gray-400 dark:text-gray-500">/</span>
                          <span className="text-xs text-gray-500 dark:text-gray-400 font-mono">{s.total}</span>
                          {s.missing > 0 && (
                            <span className="text-xs text-red-500 dark:text-red-400 font-mono ml-auto">-{s.missing}</span>
                          )}
                        </div>
                      </div>
                      <div className="w-16 text-right flex-shrink-0">
                        {s.dropRate > 0 ? (
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${
                            s.dropRate >= 50 ? 'bg-red-500/15 text-red-600 dark:text-red-400' :
                            s.dropRate > 0 ? 'bg-amber-500/15 text-amber-600 dark:text-amber-400' : ''
                          }`}>
                            {s.dropRate}%
                          </span>
                        ) : (
                          <span className="text-xs text-emerald-600/70 dark:text-emerald-500/60">0%</span>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl ring-1 ring-gray-200 dark:ring-gray-700 overflow-hidden">
              ...
              <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center gap-3">
                {/* Filter Tabs */}
                <div className="flex items-center bg-gray-100 dark:bg-gray-900/50 rounded-lg p-0.5 ring-1 ring-gray-200 dark:ring-gray-700/50">
                  {([
                    ...
                    >
                      {f.label}
                      <span className={`ml-1.5 ${filterView === f.key ? 'text-gray-500 dark:text-gray-300' : 'text-gray-400 dark:text-gray-500'}`}>
                        {f.count}
                      </span>
                    </button>
                  ))}
                ...
                <div className="flex-1 min-w-[200px] relative">
                  ...
                  <input
                    ...
                    className="w-full bg-gray-50 dark:bg-gray-900/50 ring-1 ring-gray-200 dark:ring-gray-700/50 rounded-lg pl-9 pr-3 py-1.5 text-sm text-gray-700 dark:text-gray-200 placeholder:text-gray-400 dark:placeholder:text-gray-600 focus:outline-none focus:ring-gray-400 dark:focus:ring-gray-600"
                  />
                </div>
              ...
              {/* Table Header */}
              <div className="hidden lg:grid grid-cols-[2rem_1fr_1.2fr_0.8fr_0.8fr_0.6fr_0.5fr] gap-4 px-5 py-2.5 border-b border-gray-200 dark:border-gray-700 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                <span />
              ...
              {/* Contact Rows */}
              <div className="divide-y divide-gray-200/80 dark:divide-gray-700/60">
...
function SummaryCard({
  ...
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-2xl ring-1 ring-gray-200 dark:ring-gray-700 p-5 relative overflow-hidden ${pulse ? 'ring-red-500/30' : ''} ${clickable ? 'hover:ring-red-500/50 cursor-pointer transition-all' : ''}`}>
...
function ContactRow({ contact: c, expanded, onToggle, selected, onSelect, utmSelected, onUtmSelect, showUtmCheckbox, agentOverride, onAgentToggle, isDismissed, onUndismiss }: {
  ...
  return (
    <div className="group">
      <div
        className={`w-full grid grid-cols-1 lg:grid-cols-[2rem_1fr_1.2fr_0.8fr_0.8fr_0.6fr_0.5fr] gap-2 lg:gap-4 px-5 py-3.5 text-left hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors cursor-pointer ${
          isDismissed ? 'opacity-50' : selected ? 'bg-blue-50 dark:bg-blue-500/10' : utmSelected ? 'bg-amber-50 dark:bg-amber-500/10' : ''
        }`}
        onClick={onToggle}
      >
        {/* Checkbox */}
        ...
        <div className="flex items-center gap-3">
          <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
            c.inSpark
              ? (c.matchMethod !== 'email' ? 'bg-amber-400' : c.hasUtmGap ? 'bg-amber-400' : 'bg-emerald-500')
              : (c.warnings.length > 0 ? 'bg-amber-400' : 'bg-red-500')
          }`} />
        ...
        <div className="flex flex-col gap-0.5 pl-5 lg:pl-0">
          <span className="text-sm text-gray-700 dark:text-gray-300 truncate">{c.email}</span>
          <span className="text-xs text-gray-500 dark:text-gray-500">{c.phone}</span>
        </div>
        ...
        {/* Submitted */}
        <div className="pl-5 lg:pl-0">
          <span className="text-sm text-gray-700 dark:text-gray-300">{formatDate(c.submittedAt)}</span>
          <span className="text-xs text-gray-500 dark:text-gray-500 ml-2">{c.formType}</span>
        </div>
      ...
      {/* Expanded Detail */}
      {expanded && (
        <div className="px-5 pb-4 border-t border-gray-200 dark:border-gray-700/40">
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* CallRail Side */}
            <div className="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-4 ring-1 ring-gray-200 dark:ring-gray-700/30">
            ...
            <div className={`rounded-xl p-4 ring-1 ${
              c.inSpark
                ? 'bg-emerald-50 dark:bg-emerald-500/5 ring-emerald-200 dark:ring-emerald-500/20'
                : 'bg-red-50 dark:bg-red-500/5 ring-red-200 dark:ring-red-500/20'
            }`}>
              <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-2">
  ...
  return (
    <div>
      ...
      <p className={`${mono ? 'font-mono text-xs' : 'text-sm'} ${
        empty ? 'text-gray-400 dark:text-gray-500 italic' : highlight ? 'text-amber-600 dark:text-amber-400' : 'text-gray-700 dark:text-gray-200'
      }`}>
        {empty ? '(empty)' : value}
      </p>
    </div>
