...
- Reports to date: 2026-03-11, 2026-04-25, 2026-06-04, 2026-07-14, 2026-07-27, 2026-07-30, 2026-07-full, 2026-08-13 (Jul 30–Aug 13 fortnight), 2026-09-13 (Aug 14–Sep 13, 31 days — resumed the monthly cadence; 106 leads, $194 blended CPL).
- **Extraction gotchas (2026-09-14, full detail in the report repo's `.claude/commands/regen-report.md` pre-flight §6–8):** never `navigate`/reload the Looker tab after date ranges are set — it silently resets every page to `Last 30 days`; move between pages by sidebar click only. Virtualized tables (Google Ads Insights, Meta Lead Insights, GA sessions) need 3–4-tick scrolls inside the widget with `get_page_text` after each. CallRail page: the top KPI tiles, Calls-by-Source and Answered-by-Tracking-Number are all "Unknown data source" now — the daily table, its summary tiles and the Source table still load and reconcile.
...
**Last Updated**: 2026-09-14 (Shipped the Aug 14–Sep 13 report via /regen-report; ReportsTab now points at 2026-09-13; Looker extraction gotchas recorded under Reports Page. Previous: 2026-08-31 — Snapshot cron was silently publishing rate-limit-truncated contact sets on green runs — fixed with client-level 429 retry, `throwOnError` on `listAllContacts`, abort-on-failed-detail-fetch, and a >10% shrink guard. See "Rate-limit truncation fix" under Rating History. Cron time drift accepted.)
